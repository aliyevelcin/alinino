...
def filtr_page(request):
    ...
    print(sort)
    if sort == 'new':
        az_products = Product.objects.order_by('-id')
    if sort == 'lower-price':
        az_products = Product.objects.order_by('price')
    if sort == 'higher-price':
        az_products = Product.objects.order_by('-price')
    if sort == 'discount':
        az_products = Product.objects.order_by('discount_price')
    if search:
        az_products = Product.objects.filter(title__icontains=search)
    newss = News.objects.order_by("-id")[:3]
...
def filtr_page_rus(request):
    ...
    print(sort)
    if sort == 'new':
        rus_products = Product.objects.order_by('-id')
    if sort == 'lower-price':
        rus_products = Product.objects.order_by('price')
    if sort == 'higher-price':
        rus_products = Product.objects.order_by('-price')
    if sort == 'discount':
        rus_products = Product.objects.order_by('discount_price')
    if search:
        rus_products = Product.objects.filter(title__icontains=search)
    newss = News.objects.order_by("-id")[:3]
...
def filtr_page_turk(request):
    ...
    print(sort)
    if sort == 'new':
        tr_products = Product.objects.order_by('-id')
    if sort == 'lower-price':
        tr_products = Product.objects.order_by('price')
    if sort == 'higher-price':
        tr_products = Product.objects.order_by('-price')
    if sort == 'discount':
        tr_products = Product.objects.order_by('discount_price')
    if search:
        tr_products = Product.objects.filter(title__icontains=search)
    newss = News.objects.order_by("-id")[:3]
...
def filtr_page_ing(request):
    ...
    print(sort)
    if sort == 'new':
        ing_products = Product.objects.order_by('-id')
    if sort == 'lower-price':
        ing_products = Product.objects.order_by('price')
    if sort == 'higher-price':
        ing_products = Product.objects.order_by('-price')
    if sort == 'discount':
        ing_products = Product.objects.order_by('discount_price')
    if search:
        ing_products = Product.objects.filter(title__icontains=search)
    newss = News.objects.order_by("-id")[:3]
